...
class BookingService {
  ...
  Future<ApiResponse<Booking>> customerMarkDisputed(
  ...
  Future<ApiResponse<Booking>> providerMarkDisputed(
      String bookingId, String reason) async {
  ...
  static bool isValidBookingTime(
      DateTime bookingDate, String startTime, String endTime) {
    try {
...
final bookingListProvider =
    FutureProvider.family<BookingListResponse, Map<String, dynamic>?>(
        (ref, filters) async {
  final bookingService = ref.read(bookingServiceProvider);

  final response = await bookingService.listBookings(
    role: filters?['role'] != null
        ? UserRole.values.firstWhere((e) => e.value == filters!['role'])
        : null,
    status: filters?['status'] != null
        ? BookingStatus.values.firstWhere((e) => e.value == filters!['status'])
        : null,
...
final customerBookingsProvider =
  ...
  if (response.success && response.data != null) {
    ...
    throw Exception(response.message ?? 'Failed to load customer bookings');
  }
});
