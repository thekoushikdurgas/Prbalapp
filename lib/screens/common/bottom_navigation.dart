...
class _BottomNavigationState extends ConsumerState<BottomNavigation> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint('🧭 BottomNavigation: Theme mode: ${themeManager.themeManager ? 'dark' : 'light'}');

    // Get user type specific colors using ThemeManager
    final userTypeColor = _getUserTypeThemeColor(themeManager, _userType);
    ...
    final List<NavigationItem> navigationItems = _getNavigationItemsForUserType(_userType, themeManager);
    ...
    return Scaffold(
      // backgroundColor: themeManager.transparent.withValues(alpha: 0.0),
      ...
      bottomNavigationBar: Container(
        ...
        decoration: BoxDecoration(
          // color: themeManager.transparent.withValues(alpha: 0.0),
            ...
            child: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildActiveIndicator(themeManager, isActive, size, userTypeColor),

                  // Enhanced navigation icon with glow effect
                  _buildNavigationIcon(themeManager, item, isActive, size, userTypeColor),
      ...
      builder: (context, child) {
        return AnimatedContainer(
          ...
          child: Icon(
            ...
            color: isActive ? userTypeColor : themeManager.textSecondary,
          ),
        );
      },
    );
  ...
  Color _getUserTypeThemeColor(ThemeManager themeManager, UserType userType) {
    switch (userType) {
      case UserType.provider:
        return themeManager.successColor; // Green for providers
      case UserType.admin:
        return themeManager.primaryColor; // Primary violet for admins
      case UserType.customer:
        return themeManager.infoColor; // Blue for customers
    }
  }
  ...
  List<NavigationItem> _getNavigationItemsForUserType(UserType userType, ThemeManager themeManager) {
