...
class _HealthDashboardState extends ConsumerState<HealthDashboard>
  ...
  Map<String, dynamic>? _dependenciesHealth;
  String? _metricsData;
  ...
  Future<void> _refreshHealthData() async {
    ...
    try {
      final results = await Future.wait([
        _healthService.getSystemHealth(),
        _healthService.getDatabaseHealth(),
        _healthService.getDependenciesHealth(),
        _healthService.collectMetrics(),
      ]);

      setState(() {
        _systemHealth = results[0] as Map<String, dynamic>?;
        _databaseHealth = results[1] as Map<String, dynamic>?;
        _dependenciesHealth = results[2] as Map<String, dynamic>?;
        _metricsData = results[3] as String?;
        ...
        child: SafeArea(
          child: CustomScrollView(
            slivers: [
              ...
              // Performance metrics
              SliverToBoxAdapter(
                child: _buildPerformanceMetrics(isDark),
              ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _buildStatusMetric('99.9%', 'Uptime', Colors.white),
                SizedBox(width: 24.w),
                _buildStatusMetric('< 50ms', 'Response Time', Colors.white),
                SizedBox(width: 24.w),
                _buildStatusMetric('0', 'Critical Issues', Colors.white),
              ],
            ),
          ],
        ),
  ...
  Widget _buildServiceHealth(bool isDark) {
    final services = [
      {
        'name': 'API Gateway',
        'status': _systemHealth?['status'] == 'healthy' ? 'Healthy' : 'Warning',
        'responseTime': '45ms',
      ...
      },
      {
        'name': 'Database',
        'status':
            _databaseHealth?['status'] == 'healthy' ? 'Healthy' : 'Warning',
        'responseTime': '12ms',
      ...
      },
      {
        'name': 'Dependencies',
        'status':
            _dependenciesHealth?['status'] == 'healthy' ? 'Healthy' : 'Warning',
        'responseTime': '3ms',
        'color': _dependenciesHealth?['status'] == 'healthy'
            ? const Color(0xFF10B981)
            : const Color(0xFFF59E0B)
      },
      {
